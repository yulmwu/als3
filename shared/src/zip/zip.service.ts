...
export class ZipService {
    async createZip(files: FileToZip[]): Promise<Readable> {
        if (!files || files.length === 0) throw new BadRequestException('No files provided for ZIP creation')
        const archive = archiver('zip', { zlib: { level: 9 } })
        archive.on('error', (err: any) => {
            throw new BadRequestException(`Failed to create ZIP: ${err.message}`)
        })
        for (const file of files) {
            if (Buffer.isBuffer(file.content)) archive.append(file.content, { name: file.path })
            else archive.append(file.content as Readable, { name: file.path })
        }
        archive.finalize()
    ...
    async createZipWithStructure(rootName: string, files: FileToZip[]): Promise<Readable> {
        ...
        for (const file of files) {
            ...
            if (Buffer.isBuffer(file.content)) archive.append(file.content, { name: zipPath })
            else archive.append(file.content as Readable, { name: zipPath })
        }
        archive.finalize()
        return archive as unknown as Readable
    }
    ...
    createZipStreamForDirectory(rootName: string): ZipStreamContext {
        ...
        return { archive, stream: archive as unknown as Readable, rootName }
    }
    ...
    addFilesToZip(context: ZipStreamContext, files: FileToZip[]): void {
        for (const file of files) {
            ...
            if (Buffer.isBuffer(file.content)) context.archive.append(file.content, { name: zipPath })
            else context.archive.append(file.content as Readable, { name: zipPath })
        }
    }
