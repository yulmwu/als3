...
export class StorageService {
    ...
    async uploadFile(file: MulterFile, userId: number, path: string = ''): Promise<string> {
    ...
    async uploadFileWithKey(file: MulterFile, s3Key: string): Promise<string> {
        await this.putObject(s3Key, file)
        return s3Key
    }

    private async putObject(s3Key: string, file: MulterFile): Promise<void> {
    ...
    async getPresignedUrls(s3Keys: string[], expiresIn: number = 3600): Promise<string[]> {
        return await Promise.all(s3Keys.map((key) => this.getPresignedUrl(key, expiresIn)))
    }
